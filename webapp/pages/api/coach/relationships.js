  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const athleteIds = (relationships || [])
        ...
        .map((relationship) => relationship.athlete_id);
      ...
      const [athleteRes, interventionsRes, raceRes, protocolRes, noteRes, checkinRes, loadInterventionRes, loadActivityRes] = await Promise.all([
        ...
        supabase.from('strava_activities').select('*').in('athlete_id', athleteIds).gte('start_date', new Date(Date.now() - 42 * 86400000).toISOString()),
      ]);
