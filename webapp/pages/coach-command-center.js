...
import { getCoachCommandCenterPreview } from '../lib/previews/coachCommandCenter';
...
export default function CoachCommandCenter({ previewMode = null, previewData = null }) {
  const { coachFeatures } = usePlan();
  const isPreview = Boolean(previewMode && previewData);

  // Data state
  const [profile, setProfile] = useState(previewData?.profile || null);
  const [summary, setSummary] = useState(previewData?.summary || null);
  const [relationships, setRelationships] = useState(previewData?.relationships || []);
  const [protocols, setProtocols] = useState(previewData?.protocols || []);
  const [invitations, setInvitations] = useState(previewData?.invitations || []);
  const [templates, setTemplates] = useState(previewData?.templates || []);
  const [sharedTemplates, setSharedTemplates] = useState(previewData?.sharedTemplates || []);
  const [notesMap, setNotesMap] = useState(previewData?.notesMap || {}); // keyed by athlete_id
  ...
  const [profileForm, setProfileForm] = useState({
    display_name: previewData?.profile?.display_name || '',
    bio: previewData?.profile?.bio || '',
    specialties: (previewData?.profile?.specialties || []).join(', '),
    certifications: (previewData?.profile?.certifications || []).join(', '),
  });
  ...
  useEffect(() => {
    if (isPreview || !coachFeatures) return;
  ...
  useEffect(() => {
    ...
    fetch(`/api/coach/notes?athlete_id=${openAthleteId}`)
      ...
      .catch(() => {});
  }, [isPreview, openAthleteId, notesMap]);
  ...
  const handleAddNote = useCallback(async (payload) => {
    if (isPreview) {
      ...
      setNotesMap((m) => ({
        ...m,
        [payload.athlete_id]: [nextNote, ...(m[payload.athlete_id] || [])],
      }));
      return true;
    }
  ...
  const handleAddProtocol = useCallback(async (payload) => {
    if (isPreview) {
      const athleteName = relationships.find((relationship) => relationship.athlete_id === payload.athlete_id)?.athlete?.name || 'Preview Athlete';
      const nextProtocol = {
        id: `preview-protocol-${Date.now()}`,
        athlete_id: payload.athlete_id,
        athlete: { name: athleteName },
        protocol_name: payload.protocol_name,
        protocol_type: payload.protocol_type,
        status: 'assigned',
        compliance_target: payload.compliance_target,
        start_date: payload.start_date,
        end_date: payload.end_date,
      };
      setProtocols((prev) => [nextProtocol, ...prev]);
      setSummary((current) => current ? { ...current, active_protocols: (current.active_protocols || 0) + 1 } : current);
      return true;
    }
  ...
  async function sendInvite(e) {
    ...
    if (isPreview) {
      ...
      setInviteEmail('');
      setInviteMsg('Preview invitation created.');
      return;
    }
  ...
  async function saveTemplate(e) {
    ...
    if (isPreview) {
      const nextTemplate = {
        ...
        protocol_type: templateForm.protocol_type,
      ...
      setTemplateMsg('Preview template saved.');
      return;
    }
  ...
  async function saveProfile(e) {
    ...
    if (isPreview) {
      ...
      setProfileMsg('Preview profile updated.');
      return;
    }
      ...
      <main className="min-h-screen bg-paper px-4 py-6 text-ink">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mb-6 flex items-center justify-between rounded-full border border-ink/10 bg-white/70 px-4 py-3 backdrop-blur">
            ...
            <DashboardTabs activeHref="/coach-command-center" tabs={[{ href: '/coach-command-center', label: 'Command Center' }]} />
          ...
          <section className="overflow-hidden rounded-[40px] border border-ink/10 bg-[linear-gradient(140deg,#1b2421_0%,#26332f_42%,#857056_100%)] p-6 text-white md:p-10">
            <div className="grid gap-8 lg:grid-cols-[1fr_auto]">
              <div>
                ...
                {isPreview ? (
                  <div className="mt-5 inline-flex rounded-full border border-white/15 bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/85">
                    Preview Mode · {previewData?.label || previewMode}
                  </div>
                ) : null}
              </div>
              {/* Summary stats */}
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    const previewData = getCoachCommandCenterPreview(previewMode);
    if (!previewData) {
      return {
        redirect: {
          destination: '/admin',
    ...
    console.error('[coach command center preview] failed:', error);
    return {
      redirect: {
        destination: '/admin',
        permanent: false,
