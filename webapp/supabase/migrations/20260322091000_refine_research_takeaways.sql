...
set
  plain_english_summary = 'Heat acclimation helps most when it is repeated often enough to move plasma volume and thermal tolerance, but not so aggressively that it strips quality out of the rest of the block.',
...
update public.research_library_entries
set
  plain_english_summary = 'Bicarbonate is useful when the race or workout has repeated surges, hard climbs, or long threshold pressure. It is much less useful as a generic every-session supplement.',
...
update public.research_library_entries
set
  plain_english_summary = 'Gut training works when the athlete rehearses the real carbohydrate load during real movement. The body learns the intake pattern by repetition, not by theory alone.',
...
update public.research_library_entries
set
  plain_english_summary = 'Sleep loss in ultra-endurance sport shows up in perception, decision-making, and steadiness before it always shows up in raw pace. That makes it easy to miss until the block is already compromised.',
...
update public.research_library_entries
set
  plain_english_summary = 'Altitude strategies are most effective when the athlete can accumulate enough hypoxic exposure without compromising the quality of key workouts. Exposure is only useful if training quality survives it.',
